...
**Multi-Agenten-Orchestrierung für Qoder. Null Lernkurve.**

_Lernen Sie nicht Qoder. Nutzen Sie einfach OMC._

[Loslegen](#schnellstart) • [Dokumentation](https://yeachan-heo.github.io/oh-my-qoder-website) • [Migrationsleitfaden](docs/MIGRATION.md) • [Discord](https://discord.gg/sj4exxQ9v)
...
Wenn Sie OMC über `omc --plugin-dir <path>` oder `qodercli --plugin-dir <path>` ausführen, fügen Sie `--plugin-dir-mode` zu `omc setup` hinzu (oder exportieren Sie `OMC_PLUGIN_ROOT` vorher), um zu vermeiden, dass Fähigkeiten/Agenten dupliziert werden, die das Plugin bereits zur Laufzeit bereitstellt. Siehe [Plugin directory flags Abschnitt in REFERENCE.md](./docs/REFERENCE.md#plugin-directory-flags) für eine vollständige Entscheidungsmatrix und alle verfügbaren Flags.
...
Aktivieren Sie Qoder native Teams in `~/.qoder/settings.json`:
...
> **Hinweis: Paketbenennung** — Das Projekt nutzt die Marke **oh-my-qoder** (Repo, Plugin, Befehle), aber das npm-Paket wird als [`oh-my-qoder`](https://www.npmjs.com/package/oh-my-qoder) veröffentlicht. Wenn Sie die CLI-Tools über npm/bun installieren, verwenden Sie `npm install -g oh-my-qoder`.
...
Mehrere Strategien für verschiedene Anwendungsfälle — von Team-gestützter Orchestrierung bis token-effizientem Refactoring. [Mehr erfahren →](https://yeachan-heo.github.io/oh-my-qoder-website/docs/#execution-modes)
...
| ------------- | ------------------------------------------------ | --------------------------------------------------------------- |
| `team`        | Kanonische Team-Orchestrierung                   | `/oh-my-qoder:team 3:executor "fix all TypeScript errors"` |
...
Automatische Wiederaufnahme von Qoder Sitzungen, wenn Rate Limits zurückgesetzt werden.
...
Leiten Sie Qoder Session-Ereignisse an ein [OpenClaw](https://openclaw.ai/)-Gateway weiter, um automatisierte Antworten und Workflows über Ihren OpenClaw-Agenten zu ermöglichen.
...
**Manuelle Einrichtung:** Erstellen Sie `~/.qoder/omc_config.openclaw.json`:
...
- **[Website](https://yeachan-heo.github.io/oh-my-qoder-website)** — Interaktive Anleitungen und Beispiele
...
- Qoder Max/Pro-Abonnement ODER Anthropic API-Schlüssel
...
**Kosten:** 3 Pro-Pläne (Qoder + Gemini + ChatGPT) decken alles für ca. $60/Monat ab.
...
Wenn Oh-My-Qoder Ihren Workflow verbessert, erwägen Sie ein Sponsoring:
