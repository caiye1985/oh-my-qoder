...
function findWorkspaceRoot(startDir) {
  ...
  while (true) {
    ...
    if ((0, import_fs3.existsSync)((0, import_path3.join)(cursor, WORKSPACE_MARKER))) {
      result = cursor;
      break;
    }
...
function readWorkspaceMarkerConfig(workspaceRoot) {
  try {
    const raw = (0, import_fs3.readFileSync)((0, import_path3.join)(workspaceRoot, WORKSPACE_MARKER), "utf-8").trim();
    if (!raw) return {};
...
function getOmcRoot(worktreeRoot) {
  ...
  if (customDir) {
    ...
    if (!dualDirWarnings.has(warningKey) && (0, import_fs3.existsSync)(legacyPath) && (0, import_fs3.existsSync)(centralizedPath)) {
