...
2. **Global (all projects)** - Creates `~/.qoder/AGENTS.md` for all Qoder sessions. Best for consistent behavior everywhere.

Set `CONFIG_TARGET` to `local` or `global` based on user's choice.

If `CONFIG_TARGET=global` and `~/.qoder/AGENTS.md` already exists without OMC markers, ask a second explicit question before running setup:
...
```bash
bash "${OMC_SETUP_PLUGIN_ROOT:-${QODER_PLUGIN_ROOT}}/scripts/setup-claude-md.sh" <CONFIG_TARGET> [GLOBAL_INSTALL_STYLE]
```
...
https://raw.githubusercontent.com/Yeachan-Heo/oh-my-qoder/main/docs/AGENTS.md
...
```bash
bash "${OMC_SETUP_PLUGIN_ROOT:-${QODER_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 2 <CONFIG_TARGET>
```
