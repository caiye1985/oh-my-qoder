...
This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `${QODER_CONFIG_DIR:-~/.qoder}/.omc-config.json`.
...
Config saved to: ~/.qoder/.omc-config.json
...
Check if `${QODER_CONFIG_DIR:-~/.qoder}/omc_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Config file: ~/.qoder/omc_config.hook.json
```
...
Read or create `${QODER_CONFIG_DIR:-~/.qoder}/omc_config.hook.json` and merge the new settings:
...
If `~/.qoder/omc_config.openclaw.json` exists, detect and offer migration:
...
| `{{tmuxSession}}` | tmux session name | `qoder:my-project` |
