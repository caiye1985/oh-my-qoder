...
mode (HUD shows `[OMC#X.Y.ZL]` with an `L` suffix), Qoder loads compiled
...
3. The user is about to restart Qoder and the working tree has TS edits with no rebuild.
...
> Qoder — `dist/` won't reflect the change otherwise.
...
- The user is in a Qoder session that isn't running OMC locally
  (no `L` in the HUD).
...
| `agents/**/*.md`               | Yes                    | No           |
| `commands/**/*.md`             | Yes                    | No           |
| `.qoder-plugin/plugin.json`   | Yes (on Claude restart)| No           |
...
triggers a rebuild within a second, so `restart Qoder` is all that's
needed afterwards.
