...
- No argument, `wizard`, `local`, `global`, or `--force` -> route to `/oh-my-qoder:omc-setup` with the same remaining args
- `doctor` -> route to `/oh-my-qoder:omc-doctor` with everything after the `doctor` token
- `mcp` -> route to `/oh-my-qoder:mcp-setup` with everything after the `mcp` token

Examples:

```bash
/oh-my-qoder:setup --local          # => /oh-my-qoder:omc-setup --local
/oh-my-qoder:setup doctor --json    # => /oh-my-qoder:omc-doctor --json
/oh-my-qoder:setup mcp github       # => /oh-my-qoder:mcp-setup github
```

## Notes

- `/oh-my-qoder:omc-setup`, `/oh-my-qoder:omc-doctor`, and `/oh-my-qoder:mcp-setup` remain valid compatibility entrypoints.
