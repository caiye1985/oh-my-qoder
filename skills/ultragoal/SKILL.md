...
<Purpose>
Ultragoal breaks a brief into an ordered set of goals, records start/checkpoint/blocker/failure events in a durable append-only ledger, and tells the active Qoder agent how to drive the Qoder `/goal` slash command alongside the plan. It does not — and cannot — mutate Qoder `/goal` state from the shell; it persists durable repo state and prints a model-facing handoff that the active agent must act on in-session.
</Purpose>
...
- The user wants the active Qoder `/goal` directive coordinated with the ledger so that a session restart does not lose progress
</Use_When>
...
<Why_This_Exists>
Qoder `/goal` is a session-scoped Stop hook: it blocks the session from stopping until a condition holds, and auto-clears on success. That is a great single-session execution primitive, but it loses state across sessions and does not by itself enforce a final review gate. `omq ultragoal` adds a durable plan, ledger, and gating layer so a long multi-step initiative can survive session restarts, fresh worktrees, and review iterations while still leveraging Qoder `/goal` to keep the active agent focused.
</Why_This_Exists>
...
1. Create a plan from a brief:
   ...
   ```
   The default mode is `aggregate` (one Qoder `/goal` covers the run).
   Pass `--qoder-goal-mode per-story` if you want each story to have its own `/goal`.
...
2. Start (or resume) the next story:
   ...
   ```
   This prints a model-facing handoff. The active Qoder agent must read it and:
...
4. If the final review is not clean, do NOT mark complete. Record blockers:
   ...
   omq ultragoal record-review-blockers --goal-id G00X-... \
     ...
     --qoder-goal-json '{"goal":{"objective":"...","status":"active"}}'
   ```
   This appends a new blocker story and keeps the Qoder `/goal` active.
...
- The shell cannot invoke or mutate Qoder `/goal` state. `omq ultragoal` only persists durable artifacts and prints instructions that the active Qoder agent reads and acts on in-session.
- Snapshots passed via `--qoder-goal-json` are model-supplied proof of the active `/goal` state; OMC validates them for textual consistency with the plan's expected objective and ledger event, but it cannot independently observe Qoder `/goal` state.
- If the Qoder `/goal` slash command is renamed or restructured, only the handoff wording needs to change; the reconciliation logic is name-agnostic.
</Important_Limitations>
