 ...
 */
export function isQoderWorktreeAvailable(): boolean {
  try {
 ...
 */
export function buildQoderWorktreeCommand(name: string, task?: string, _cwd?: string): string[] {
  const args: string[] = ['--worktree', name];
  if (task) {
    args.push(task);
  }
  args.push('--yolo');
 ...
 */
export function getQoderWorktreePath(name: string): string | null {
  try {
