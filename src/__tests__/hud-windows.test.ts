...
describe('HUD Windows Compatibility', () => {
  ...
  describe('Cross-Platform Plugin Cache Path (#670)', () => {
    it('getPluginCacheBase should return path with correct segments', () => {
      ...
      expect(normalized).toContain('plugins/cache/omc/oh-my-qoder');
    });

    it('getPluginCacheBase should use platform-native separators', () => {
      const cachePath = getPluginCacheBase();
      // On Windows: backslashes, on Unix: forward slashes
      expect(cachePath).toContain(`plugins${sep}cache${sep}omc${sep}oh-my-qoder`);
    });
    ...
    it('shared HUD wrapper template should respect QODER_CONFIG_DIR for plugin cache base', () => {
      const templatePath = join(packageRoot, 'scripts', 'lib', 'hud-wrapper-template.txt');
      const content = readFileSync(templatePath, 'utf-8');
      expect(content).toContain('getQoderConfigDir()');
    ...
    it('omc-doctor skill should use cross-platform Node.js commands', () => {
      ...
      expect(content).toContain("p.join(d,'plugins','cache','omc','oh-my-qoder')");
      expect(content).not.toContain('ls ~/.qoder/CLAUDE-*.md');
      expect(content).toContain("find \"${QODER_CONFIG_DIR:-$HOME/.claude}\" -maxdepth 1 -type f -name 'CLAUDE-*.md' -print 2>/dev/null");
    });
