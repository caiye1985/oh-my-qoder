...
describe('omc-doctor skill (issue #2254)', () => {
  it('documents AGENTS.md OMC version drift check against cached plugin version', () => {
    const skillPath = join(process.cwd(), 'skills', 'omc-doctor', 'SKILL.md');
    const content = readFileSync(skillPath, 'utf8');

    expect(content).toContain('AGENTS.md OMC version:');
    ...
    expect(content).toContain('If `AGENTS.md OMC version` != `Latest cached plugin version`: WARN - version drift detected');
  });
});
