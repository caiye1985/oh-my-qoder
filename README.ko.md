...
| `/ccg` | ask-codex + ask-gemini | Codex+Gemini 조언을 Qoder가 통합 |
...
<h1 align="center">당신의 Qoder가 스테로이드를 맞았습니다.</h1>
...
| 모드 | 특징 | 용도 |
|------|---------|---------|
| **Team (권장)** | 단계별 파이프라인 | 공유 작업 목록에서 협력하는 Qoder 에이전트 |
| **omc team (CLI)** | tmux CLI 워커 | Codex/Gemini CLI 작업; 요청 시 실행, 완료 후 종료 |
| **ccg** | 트라이-모델 병렬 | Codex(분석) + Gemini(디자인), Qoder가 통합 |
...
| `omc team` | tmux CLI 워커 (codex/gemini/qoder) | `omc team 2:codex "security review"` |
...
| `stop` | Qoder 응답 완료 시 | `{{sessionId}}`, `{{projectName}}` |
| `keyword-detector` | 프롬프트 제출마다 | `{{prompt}}`, `{{sessionId}}` |
| `ask-user-question` | Qoder가 사용자 입력 요청 시 | `{{question}}`, `{{sessionId}}` |
