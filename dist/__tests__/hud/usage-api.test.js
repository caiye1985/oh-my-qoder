...
describe('getUsage routing', () => {
    ...
    let httpsModule;
    const expectedServiceName = (configDir) => `Qoder-credentials-${createHash('sha256').update(configDir).digest('hex').slice(0, 8)}`;
    beforeAll(() => {
    ...
    it('uses the raw ~-prefixed QODER_CONFIG_DIR value for Keychain service lookup', async () => {
        process.env.QODER_CONFIG_DIR = '~/.qoder-personal';
