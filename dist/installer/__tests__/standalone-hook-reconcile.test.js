...
describe('install() standalone hook reconciliation', () => {
    beforeEach(() => {
        ...
        writeFileSync(join(testHomeDir, 'AGENTS.md'), '# test home claude');
    ...
    it('restores OMC settings hooks for standalone installs during forced reconciliation', async () => {
        ...
        expect(readFileSync(join(testClaudeDir, 'hud', 'omc-hud.mjs'), 'utf-8')).toContain('const { getQoderConfigDir } = await import(pathToFileURL(join(__dirname, "lib", "config-dir.mjs")).href);');
        expect(readFileSync(join(testClaudeDir, 'hud', 'lib', 'config-dir.mjs'), 'utf-8')).toContain('export function getQoderConfigDir()');
        expect(readFileSync(join(testClaudeDir, 'hooks', 'lib', 'config-dir.mjs'), 'utf-8')).toContain('export function getQoderConfigDir()');
    ...
    it('preserves non-OMC ~/.qoder/hooks commands while adding standalone OMC hooks', async () => {
        const settingsPath = join(testClaudeDir, 'settings.json');
        mkdirSync(testClaudeDir, { recursive: true });
    ...
    it('removes legacy OMC settings hooks in plugin mode without re-injecting them', async () => {
        const settingsPath = join(testClaudeDir, 'settings.json');
        const pluginRoot = join(testClaudeDir, 'plugins', 'cache', 'omc', 'oh-my-qoder', '4.1.5');
