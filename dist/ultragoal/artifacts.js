...
import { formatQoderGoalReconciliation, parseQoderGoalSnapshot, reconcileQoderGoalSnapshot, } from '../goal-workflows/qoder-goal-snapshot.js';
...
async function canReconcileCompletedTaskScopedAggregateSnapshot(cwd, plan, goal, snapshotObjective, evidence) {
    if (qoderGoalMode(plan) !== 'aggregate')
        return false;
...
export async function createUltragoalPlan(cwd, options) {
    ...
    if (plan.qoderGoalMode === 'aggregate')
        plan.qoderObjective = aggregateQoderObjective(candidates, planId);
    await mkdir(ultragoalDir(cwd, planId), { recursive: true });
...
export async function checkpointUltragoal(cwd, options) {
    ...
    if (options.status === 'complete') {
        ...
        const snapshot = options.qoderGoal === undefined ? null : parseQoderGoalSnapshot(options.qoderGoal);
        const reconciliation = reconcileQoderGoalSnapshot(snapshot, {
            expectedObjective,
        ...
        if (finalRunCheckpoint && !options.allowActiveFinalQoderGoal)
...
export async function recordFinalReviewBlockers(cwd, options) {
    ...
    const expectedObjective = expectedQoderObjective(plan, goal);
    const aggregateMode = qoderGoalMode(plan) === 'aggregate';
    const reconciliation = reconcileQoderGoalSnapshot(options.qoderGoal === undefined ? null : parseQoderGoalSnapshot(options.qoderGoal), {
        expectedObjective,
...
export function buildQoderGoalInstruction(goal, plan) {
    if (qoderGoalMode(plan) === 'aggregate')
        return buildAggregateQoderGoalInstruction(goal, plan);
...
function buildPerStoryQoderGoalInstruction(goal, plan) {
    ...
    return [
        ...
        finalStory
            ? `  omc ultragoal record-review-blockers --goal-id ${goal.id} --title "Resolve final code-review blockers" --objective "<blocker-resolution objective>" --evidence "<review findings>" --qoder-goal-json "<active /goal snapshot JSON or path>"`
            : `  omc ultragoal checkpoint --goal-id ${goal.id} --status complete --evidence "<tests/files/PR evidence>" --qoder-goal-json "<fresh /goal snapshot JSON or path>"`,
        finalStory
        ...
        finalStory
            ...
            : null,
        finalStory
            ? `  omc ultragoal checkpoint --goal-id ${goal.id} --status complete --evidence "<tests/files/PR evidence>" --qoder-goal-json "<fresh complete /goal snapshot JSON or path>" --quality-gate-json "<quality gate JSON or path>"`
            : null,
...
function buildAggregateQoderGoalInstruction(goal, plan) {
    ...
    return [
        ...
        finalStory
            ...
            : '- This is not the final story: do not clear the /goal yet; the aggregate Qoder /goal must remain active while later OMC stories remain.',
        finalStory
